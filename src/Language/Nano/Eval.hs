...
eval env (EBin opr e1 e2) = evalOp opr (eval env e1) (eval env e2)

eval env (ELet id exp1 exp2)  = eval env1 exp2
  where
    v    = eval env exp1
    env1 = (id,v):env
...
--(d) 
eval env (ELam f arg) = VClos env f arg
eval env (EApp f arg) = eval bodyEnv body
  where
    (VClos clEnv x body) = eval env f
    vArg = eval env arg
    bodyEnv = (x,vArg):clEnv
...
evalOp Eq (VInt x1) (VInt x2) = VBool (x1 - x2 == 0)
evalOp Eq (VBool x1) (VBool x2) = VBool (x1 == x2)
evalOp Ne (VInt x1) (VInt x2) = VBool (x1 - x2 /= 0)
...
lookupId x ((a,b):remaining)
  | x == a = b
  | otherwise = lookupId x remaining 
